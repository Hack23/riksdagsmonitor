...
/** Shape of production stats JSON */
  ...
  describe('Output Generation', () => {
    ...
    it('should create JSON output with proper structure', () => {
      ...
      if (fs.existsSync(statsPath)) {
        const stats = JSON.parse(fs.readFileSync(statsPath, 'utf-8')) as ProductionStats;
  ...
  describe('Data Validation', () => {
    it('should validate extracted statistics are numbers', () => {
      ...
      if (fs.existsSync(statsPath)) {
        const stats = JSON.parse(fs.readFileSync(statsPath, 'utf-8')) as ProductionStats;
        if (stats.counts) {
          Object.values(stats.counts).forEach((value: number) => {
