...
/** Committee report from MCP server */
...
/** Committee reports validation result */
...
/** Extended generation result for committee reports */
interface CommitteeReportsGenerationResult extends Omit<GenerationResult, 'crossReferences' | 'articles'> {
  readonly articles: readonly GeneratedArticle[];
  readonly crossReferences: CommitteeReportsCrossReferences;
}

/** Shape of the dynamically imported committee-reports module */
interface CommitteeReportsModule {
  readonly REQUIRED_TOOLS: readonly string[];
  readonly generateCommitteeReports: (options?: {
    languages?: Language[];
    limit?: number;
    writeArticle?: (html: string, filename: string) => void;
  }) => Promise<CommitteeReportsGenerationResult>;
  readonly validateCommitteeReports: (article: ArticleInput) => CommitteeReportsValidationResult;
}

// Mock MCP client
const { mockClientInstance, mockCommitteeReports, MockMCPClient } = vi.hoisted(() => {
  const mockCommitteeReports: CommitteeReport[] = [
...
let committeeReportsModule: CommitteeReportsModule;

beforeAll(async () => {
  committeeReportsModule = await import('../../scripts/news-types/committee-reports.js') as unknown as CommitteeReportsModule;
});
