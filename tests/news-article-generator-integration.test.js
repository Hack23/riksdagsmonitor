...
describe('News Article Generator Integration', () => {
  ...
  describe('Cross-Reference Validation', () => {
    it('should validate week-ahead cross-references', () => {
      ...
      const validation = validateCrossReferences('week-ahead', articleContent, mcpCalls);
    ...
    it('should validate committee-reports cross-references', () => {
      ...
      const validation = validateCrossReferences('committee-reports', articleContent, mcpCalls);
  ...
  describe('Required Tools Configuration', () => {
    it('should define required tools for all article types', () => {
      const articleTypes = ['week-ahead', 'committee-reports', 'propositions', 'motions', 'breaking'];
      
      articleTypes.forEach(type => {
        expect(REQUIRED_TOOLS_PER_TYPE[type]).toBeDefined();
        expect(Array.isArray(REQUIRED_TOOLS_PER_TYPE[type])).toBe(true);
        expect(REQUIRED_TOOLS_PER_TYPE[type].length).toBeGreaterThan(0);
      });
    });

    it('should require minimum 3 sources per article type', () => {
      Object.values(REQUIRED_TOOLS_PER_TYPE).forEach(tools => {
  ...
  describe('Cross-Reference Patterns', () => {
    it('should extract calendar references from content', async () => {
      const { extractCrossReferences } = await import('../scripts/validate-cross-references.js');
  ...
  describe('Batch Validation', () => {
    it('should validate multiple articles at once', async () => {
      const { validateArticleBatch } = await import('../scripts/validate-cross-references.js');
  ...
  describe('Validation Reporting', () => {
    it('should generate markdown validation report', async () => {
      const { generateValidationReport } = await import('../scripts/validate-cross-references.js');
    ...
    it('should export CI-friendly summary', async () => {
      const { exportCISummary } = await import('../scripts/validate-cross-references.js');
