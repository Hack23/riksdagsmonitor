...
describe('Generate News Indexes', () => {
  ...
  describe('generateAllIndexes', () => {
    ...
    // ─── Phase 2 UI features (filter bar, recency, skeletons, RTL) ─────────
    // These assertions all read the same generated HTML, so we run
    // `generateAllIndexes()` exactly once in `beforeAll` and cache the
    // English / Swedish / Arabic outputs to avoid regenerating ~14
    // language-index files for every individual `it()`.
