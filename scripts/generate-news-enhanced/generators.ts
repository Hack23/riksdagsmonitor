...
function buildExecutiveSummary(docs: RawDocument[], topic: string | null, lang: Language): string {
  ...
  const templates: Partial<Record<Language, string>> = {
    ...
    da: `Denne dybdeanalyse undersøger ${docs.length} parlamentariske dokumenter${topic ? ` om <strong>${esc(topic)}</strong>` : ''}${domainPhrase ? ` inden for ${domainPhrase}` : ''}. ${enriched} af disse er beriget med fulde tekster. Den lovgivningsmæssige holdning er ${govLed === null ? 'ikke-lovgivningsmæssig' : govLed ? 'regeringsdrevet' : 'oppositionsdrevet'}${propCount + betCount > 0 ? ` med ${propCount} forslag og ${betCount} udvalgsrapport${betCount !== 1 ? 'er' : ''}` : ''}. Beslutningstagere bør følge udvalgsdrøftelser og afstemninger.`,
    no: `Denne dybdeanalysen undersøker ${docs.length} parlamentariske dokumenter${topic ? ` om <strong>${esc(topic)}</strong>` : ''}${domainPhrase ? ` innen ${domainPhrase}` : ''}. ${enriched} av disse er beriket med fulltekst. Den lovgivningsmessige posisjonen er ${govLed === null ? 'ikke-lovgivningsmessig' : govLed ? 'regjeringsledet' : 'opposisjonsdrevet'}${propCount + betCount > 0 ? ` med ${propCount} forslag og ${betCount} komitérapport${betCount !== 1 ? 'er' : ''}` : ''}. Beslutningstakere bør følge komitéforhandlinger og voteringsmønstre.`,
    fi: `Tämä syväanalyysi tutkii ${docs.length} parlamentaarista asiakirjaa${topic ? ` aiheesta <strong>${esc(topic)}</strong>` : ''}${domainPhrase ? ` alueilla ${domainPhrase}` : ''}. Näistä ${enriched} rikastettiin koko tekstillä. Lainsäädäntöasenne on ${govLed === null ? 'ei-lainsäädännöllinen' : govLed ? 'hallitusvetoinen' : 'oppositiovetoinen'}${propCount + betCount > 0 ? ` — ${propCount} esitystä ja ${betCount} valiokunnan mietintöä` : ''}. Päätöksentekijöiden tulisi seurata valiokuntien harkintaa ja äänestyksiä.`,
    ...
    ja: `この詳細分析レポートは${docs.length}件の議会文書${topic ? `（<strong>${esc(topic)}</strong>に関する）` : ''}${domainPhrase ? `（${domainPhrase}分野）` : ''}を分析します。${enriched}件は全文で強化されています。立法スタンスは${govLed === null ? '非立法的' : govLed ? '政府主導' : '野党主導'}${propCount + betCount > 0 ? `で、${propCount}件の提案と${betCount > 0 ? `${betCount}件の委員会報告` : '委員会報告なし'}があります` : 'です'}。意思決定者は委員会審議と投票パターンを追跡する必要があります。`,
    ko: `이 심층 분석 보고서는 ${docs.length}개의 의회 문서${topic ? `（<strong>${esc(topic)}</strong> 관련）` : ''}${domainPhrase ? `（${domainPhrase} 분야）` : ''}를 분석합니다. 이 중 ${enriched}개는 전문으로 보강되었습니다. 입법 태도는 ${govLed === null ? '비입법적' : govLed ? '정부 주도' : '야당 주도'}${propCount + betCount > 0 ? `이며, ${propCount}개 제안과 ${betCount > 0 ? `${betCount}개 위원회 보고서` : '위원회 보고서 없음'}가 있습니다` : '입니다'}. 의사결정자는 위원회 심의와 투표 패턴을 추적해야 합니다.`,
