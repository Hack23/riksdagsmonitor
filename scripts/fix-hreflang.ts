#!/usr/bin/env npx tsx
/**
 * Fix hreflang links in news HTML files
 *
 * Repairs two issues found in older news articles generated by agentic
 * workflows before the `renderHreflangBlock` helper was standardised:
 *
 * 1. **Relative hreflang URLs** → converts to absolute URLs with
 *    `https://riksdagsmonitor.com/news/` prefix
 * 2. **Missing `x-default`** → adds `x-default` pointing at the English
 *    version (or the file itself if no English sibling exists)
 * 3. **Non-standard hreflang codes** → normalises `zh-CN` → `zh` (the
 *    site uses plain `zh` throughout)
 *
...
function main(): void {
  if (!fs.existsSync(NEWS_DIR)) {
    console.log('No news/ directory found — nothing to fix.');
    return;
  }

  const htmlFiles = collectHtmlFiles(NEWS_DIR);

  console.log(`Scanning ${htmlFiles.length} news HTML files…`);
  ...
  if (results.length === 0) {
    console.log('\n✅ All news files have correct hreflang links.');
    return;
  }
