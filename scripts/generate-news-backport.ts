 ...
 * activities, government actions, and committee developments.
 *
...
type TitleMapWeekAhead = Readonly<Record<Language, (from: string, to: string) => TitleInfo>>;
type TitleMapCount = Readonly<Record<Language, (n: number) => TitleInfo>>;
...
const fromDate: string = getArg('from', '2026-01-05');
const toDate: string = getArg('to', new Date().toISOString().split('T')[0]);
const mode: string = getArg('mode', 'weekly');
const typesInput: string = getArg('types', 'week-ahead,committee-reports,propositions,motions');
const dryRun: boolean = args.includes('--dry-run');
const delayMs: number = parseInt(getArg('delay', '2000'), 10);

// Language support
const ALL_LANGUAGES: readonly Language[] = ['en', 'sv', 'da', 'no', 'fi', 'de', 'fr', 'es', 'nl', 'ar', 'he', 'ja', 'ko', 'zh'];
const LANGUAGE_PRESETS: Readonly<Record<string, readonly Language[]>> = {
  all: ALL_LANGUAGES,
  nordic: ['en', 'sv', 'da', 'no', 'fi'],
  'eu-core': ['en', 'sv', 'de', 'fr', 'es', 'nl'],
};

let langInput: string = getArg('languages', 'en,sv').trim().toLowerCase();
const preset = LANGUAGE_PRESETS[langInput];
if (preset) {
  langInput = preset.join(',');
}
const languages: Language[] = langInput
  .split(',')
  .map((l) => l.trim())
  .filter((l): l is Language => (ALL_LANGUAGES as readonly string[]).includes(l));

const articleTypes: string[] = typesInput.split(',').filter(Boolean);
...
function getMondaysInRange(from: string, to: string): Date[] {
  const mondays: Date[] = [];
...
function articleExists(slug: string, lang: Language): boolean {
...
const WEEK_AHEAD_TITLES: TitleMapWeekAhead = {
...
const COMMITTEE_TITLES: TitleMapCount = {
...
const PROPOSITION_TITLES: TitleMapCount = {
...
const MOTION_TITLES: TitleMapCount = {
...
async function generateForDate(targetDate: Date, type: string, client: MCPClient): Promise<number> {
  const dateStr = formatDate(targetDate);
  const weekEnd = formatDate(addDays(targetDate, 6));
  let slug: string;
  let titleMap: TitleMapWeekAhead | TitleMapCount;
  ...
  switch (type) {
    case 'week-ahead': {
      ...
      const events = await client.fetchCalendarEvents(dateStr, weekEnd);
      data = { events };
      dataKey = 'week-ahead';
  ...
  for (const lang of missingLangs) {
    try {
      ...
      if (type === 'week-ahead') {
        const fn = (titleMap as TitleMapWeekAhead)[lang] || (titleMap as TitleMapWeekAhead).en;
        titles = fn(dateStr, weekEnd);
      } else {
        const fn = (titleMap as TitleMapCount)[lang] || (titleMap as TitleMapCount).en;
        titles = fn(count);
      }
      ...
      console.error(`      ❌ ${lang}: ${(err as Error).message}`);
    }
  }
  ...
  for (const date of dates) {
    ...
    for (const type of articleTypes) {
      ...
      try {
        ...
        console.error(`  ❌ Error for ${type} on ${dateStr}: ${(err as Error).message}`);
        totalErrors++;
      }
    }
  }
