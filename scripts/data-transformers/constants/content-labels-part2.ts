...
export const CONTENT_LABELS_PART2: Partial<Record<string, ContentLabelSet>> = {
  es: {
    ...
    interpellationDefault: 'Interpelación parlamentaria que exige rendición de cuentas del gobierno.',
    readFullInterpellation: 'Leer la interpelación completa',
    ...
    noInterpellations: 'No hay interpelaciones disponibles en este momento.',
    interpellationsBreakdown: (n: number): string => `Los diputados de la oposición han presentado ${n} interpelaciones a ministros del gobierno, exigiendo rendición de cuentas y respuestas formales sobre cuestiones políticas urgentes.`,
    debateDynamics: 'Dinámica del debate y responsabilidad',
    ministerAccountability: 'Dirigida a',
    accountabilityAnalysis: 'Análisis de responsabilidad',
    interpellationBy: 'Interpelación de',
    interpellationsScrutinyContext: (n: number): string => `Interpelaciones de ${n} partidos diferentes demuestran un amplio escrutinio parlamentario de los ministros del gobierno.`,
    interpellationsAccountabilityContext: (count: number, domains: number): string => `Estas ${count} interpelaciones abarcan ${domains} ámbito${domains > 1 ? 's' : ''} de política, reflejando la amplitud de las demandas parlamentarias de responsabilidad al gobierno.`,
    ...
    interpellationStrategyContext: (n: number): string => `Interpelaciones de ${n} partidos diferentes demuestran un control parlamentario coordinado y exigencias de rendición de cuentas gubernamental.`,
    ...
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} interpelación${n > 1 ? 'es' : ''} presentada${n > 1 ? 's' : ''}`,
  ...
  nl: {
    ...
    interpellationDefault: 'Parlementaire interpellatie die verantwoording van de regering eist.',
    readFullInterpellation: 'Lees de volledige interpellatie',
    ...
    noInterpellations: 'Geen interpellaties beschikbaar op dit moment.',
    interpellationsBreakdown: (n: number): string => `Oppositieleden hebben ${n} interpellaties ingediend bij regeringsministers en eisen verantwoording en formele antwoorden op dringende politieke kwesties.`,
    debateDynamics: 'Debatdynamiek en verantwoording',
    ministerAccountability: 'Gericht aan',
    accountabilityAnalysis: 'Verantwoordingsanalyse',
    interpellationBy: 'Interpellatie door',
    interpellationsScrutinyContext: (n: number): string => `Interpellaties van ${n} verschillende partijen tonen brede parlementaire controle op de ministers van de regering.`,
    interpellationsAccountabilityContext: (count: number, domains: number): string => `Deze ${count} interpellaties bestrijken ${domains} beleidsterrein${domains > 1 ? 'en' : ''}, wat de breedte van parlementaire verantwoordingseisen aan de regering weerspiegelt.`,
    ...
    interpellationStrategyContext: (n: number): string => `Interpellaties van ${n} verschillende partijen tonen gecoördineerde parlementaire controle en eisen van regeringsverantwoordelijkheid.`,
    ...
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} interpellatie${n > 1 ? 's' : ''} ingediend`,
  ...
  ar: {
    ...
    interpellationDefault: 'استجواب برلماني يطالب بمساءلة الحكومة.',
    readFullInterpellation: 'قراءة الاستجواب الكامل',
    ...
    noInterpellations: 'لا توجد استجوابات متاحة حالياً.',
    interpellationsBreakdown: (n: number): string => `قدم نواب المعارضة ${n} استجواباً لوزراء الحكومة، مطالبين بالمساءلة وردود رسمية على القضايا السياسية الملحّة.`,
    debateDynamics: 'ديناميكيات النقاش والمساءلة',
    ministerAccountability: 'موجه إلى',
    accountabilityAnalysis: 'تحليل المساءلة',
    interpellationBy: 'استجواب من',
    interpellationsScrutinyContext: (n: number): string => `استجوابات من ${n} أحزاب مختلفة تُظهر رقابة برلمانية واسعة على وزراء الحكومة.`,
    interpellationsAccountabilityContext: (count: number, domains: number): string => `تغطي هذه الاستجوابات الـ ${count} عدد ${domains} من مجالات السياسة، مما يعكس اتساع مطالب المساءلة البرلمانية تجاه الحكومة.`,
    ...
    interpellationStrategyContext: (n: number): string => `استجوابات من ${n} أحزاب مختلفة تُظهر رقابة برلمانية منسقة ومطالب بالمساءلة الحكومية.`,
    ...
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} استجواب${n > 1 ? 'ات' : ''} مقدمة`,
  ...
  he: {
    ...
    interpellationDefault: 'אינטרפלציה פרלמנטרית הדורשת אחריותיות מהממשלה.',
    readFullInterpellation: 'קראו את האינטרפלציה המלאה',
    ...
    noInterpellations: 'אין אינטרפלציות זמינות כרגע.',
    interpellationsBreakdown: (n: number): string => `חברי האופוזיציה הגישו ${n} אינטרפלציות לשרי הממשלה, ודורשים אחריות ותשובות רשמיות על סוגיות מדיניות דחופות.`,
    debateDynamics: 'דינמיקת הדיון ואחריות',
    ministerAccountability: 'מופנית אל',
    accountabilityAnalysis: 'ניתוח אחריות',
    interpellationBy: 'אינטרפלציה מאת',
    interpellationsScrutinyContext: (n: number): string => `אינטרפלציות מ-${n} מפלגות שונות מדגימות פיקוח פרלמנטרי רחב על שרי הממשלה.`,
    interpellationsAccountabilityContext: (count: number, domains: number): string => `${count} אינטרפלציות אלה מכסות ${domains} תחומי מדיניות, המשקפים את רוחב דרישות האחריותיות הפרלמנטריות כלפי הממשלה.`,
    ...
    interpellationStrategyContext: (n: number): string => `אינטרפלציות מ-${n} מפלגות שונות מדגימות פיקוח פרלמנטרי מתואם ודרישות לאחריותיות ממשלתית.`,
    ...
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} אינטרפלצי${n > 1 ? 'ות' : 'ה'} הוגש${n > 1 ? 'ו' : 'ה'}`,
  ...
  ja: {
    ...
    interpellationDefault: '政府の説明責任を求める国会質問主意書。',
    readFullInterpellation: '質問主意書の全文を読む',
    policyContext: '政策的背景',
    filedBy: '提出者',
    published: '公開日',
    ...
    noInterpellations: '現在、質問主意書はありません。',
    interpellationsBreakdown: (n: number): string => `野党議員が閣僚に対して${n}件の質問主意書を提出し、緊急の政策問題に関する説明責任と正式な回答を求めています。`,
    debateDynamics: '討論の動態と説明責任',
    ministerAccountability: '宛先',
    accountabilityAnalysis: '説明責任分析',
    interpellationBy: '質問主意書提出者',
    interpellationsScrutinyContext: (n: number): string => `${n}の異なる政党からの質問主意書は、政府大臣に対する広範な議会の監視を示しています。`,
    interpellationsAccountabilityContext: (count: number, domains: number): string => `これら${count}件の質問主意書は${domains}の政策分野にまたがり、政府に対する議会の説明責任要求の幅広さを反映しています。`,
    ...
    interpellationStrategyContext: (n: number): string => `${n}つの異なる政党からの質問主意書は、協調的な議会監視と政府の説明責任への要求を示しています。`,
    ...
    partyInterpellationsFiled: (party: string, n: number): string => `${party}：${n}件の質問主意書を提出`,
  ...
  ko: {
    ...
    interpellationDefault: '정부의 책임을 요구하는 국회 대정부질문.',
    readFullInterpellation: '전체 대정부질문 읽기',
    policyContext: '정책 맥락',
    filedBy: '제출자',
    published: '게시일',
    ...
    noInterpellations: '현재 이용 가능한 대정부질문이 없습니다.',
    interpellationsBreakdown: (n: number): string => `야당 의원들이 정부 장관에게 ${n}건의 대정부질문을 제출하여 긴박한 정책 문제에 대한 책임과 공식 답변을 요구하고 있습니다.`,
    debateDynamics: '토론 역학 및 책임성',
    ministerAccountability: '수신자',
    accountabilityAnalysis: '책임성 분석',
    interpellationBy: '질의 제출자',
    interpellationsScrutinyContext: (n: number): string => `${n}개의 서로 다른 정당의 대정부 질의는 정부 장관에 대한 광범위한 의회 감시를 보여줍니다.`,
    interpellationsAccountabilityContext: (count: number, domains: number): string => `이 ${count}건의 대정부 질의는 ${domains}개의 정책 분야에 걸쳐 있으며, 정부에 대한 의회 책임 요구의 폭을 반영합니다.`,
    ...
    interpellationStrategyContext: (n: number): string => `${n}개 정당의 대정부질문은 조율된 의회 감시와 정부 책임 요구를 보여줍니다.`,
    ...
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n}건의 대정부질문 제출`,
  ...
  zh: {
    ...
    interpellationDefault: '要求政府问责的议会质询。',
    readFullInterpellation: '阅读完整质询',
    policyContext: '政策背景',
    filedBy: '提交者',
    published: '发布日期',
    ...
    noInterpellations: '目前没有可用的质询。',
    interpellationsBreakdown: (n: number): string => `反对党议员向政府部长提交了${n}份质询，要求对紧迫政策问题承担责任并作出正式答复。`,
    debateDynamics: '辩论动态与问责制',
    ministerAccountability: '质询对象',
    accountabilityAnalysis: '问责分析',
    interpellationBy: '质询提出者',
    interpellationsScrutinyContext: (n: number): string => `来自${n}个不同政党的质询展示了对政府部长的广泛议会监督。`,
    interpellationsAccountabilityContext: (count: number, domains: number): string => `这${count}项质询涵盖${domains}个政策领域，反映了议会对政府问责要求的广度。`,
    ...
    interpellationStrategyContext: (n: number): string => `来自${n}个不同政党的质询显示了协调的议会监督和对政府问责的要求。`,
    ...
    partyInterpellationsFiled: (party: string, n: number): string => `${party}：提交了${n}项质询`,
