...
export const CONTENT_LABELS_PART2: Partial<Record<string, ContentLabelSet>> = {
  es: {
    ...
    noInterpellations: 'No hay interpelaciones disponibles en este momento.',
    interpellationsBreakdown: (n: number): string => `Los diputados de la oposición han presentado ${n} interpelaciones exigiendo responsabilidad ministerial.`,
    ministerAccountability: 'Responsabilidad ministerial',
    ministerAccountabilityContext: (n: number): string => `${n} ministros se enfrentan a interpelaciones parlamentarias en este período, destacando las prioridades de control de la oposición.`,
    targetMinister: 'Ministro destinatario',
    interpellationBy: 'Presentada por',
    questioner: 'Interpelante',
    oppositionOversight: 'Análisis del control de la oposición',
    readFullInterpellation: 'Ver interpelación',
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} interpelación${n > 1 ? 'es' : ''} presentada${n > 1 ? 's' : ''}`,
  ...
  nl: {
    ...
    noInterpellations: 'Geen interpellaties beschikbaar op dit moment.',
    interpellationsBreakdown: (n: number): string => `Oppositieleden hebben ${n} interpellaties ingediend die ministeriële verantwoordingsplicht eisen.`,
    ministerAccountability: 'Ministeriële verantwoordingsplicht',
    ministerAccountabilityContext: (n: number): string => `${n} ministers worden geconfronteerd met parlementaire interpellaties in deze periode, wat de controleprioriteiten van de oppositie benadrukt.`,
    targetMinister: 'Doelminister',
    interpellationBy: 'Ingediend door',
    questioner: 'Vraagsteller',
    oppositionOversight: 'Analyse van oppositioneel toezicht',
    readFullInterpellation: 'Bekijk interpellatie',
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} interpellatie${n > 1 ? 's' : ''} ingediend`,
  ...
  ar: {
    ...
    noInterpellations: 'لا تتوفر استجوابات في الوقت الحالي.',
    interpellationsBreakdown: (n: number): string => `قدّم نواب المعارضة ${n} استجواباً يطالبون فيه بمساءلة الوزراء.`,
    ministerAccountability: 'المساءلة الوزارية',
    ministerAccountabilityContext: (n: number): string => `يواجه ${n} وزراء استجوابات برلمانية في هذه الفترة، مما يسلط الضوء على أولويات رقابة المعارضة.`,
    targetMinister: 'الوزير المستجوَب',
    interpellationBy: 'قُدِّم من قِبَل',
    questioner: 'المستجوِب',
    oppositionOversight: 'تحليل رقابة المعارضة',
    readFullInterpellation: 'عرض الاستجواب',
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} استجواب${n > 1 ? 'ات' : ''} مقدمة`,
  ...
  he: {
    ...
    noInterpellations: 'אין אינטרפלציות זמינות כרגע.',
    interpellationsBreakdown: (n: number): string => `חברי האופוזיציה הגישו ${n} אינטרפלציות הדורשות אחריות שרים.`,
    ministerAccountability: 'אחריות שרים',
    ministerAccountabilityContext: (n: number): string => `${n} שרים מתמודדים עם אינטרפלציות פרלמנטריות בתקופה זו, המדגישות את סדרי העדיפויות של האופוזיציה.`,
    targetMinister: 'שר יעד',
    interpellationBy: 'הוגש על ידי',
    questioner: 'מציג השאלה',
    oppositionOversight: 'ניתוח פיקוח אופוזיציה',
    readFullInterpellation: 'צפה באינטרפלציה',
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n} אינטרפלצי${n > 1 ? 'ות' : 'ה'} הוגש${n > 1 ? 'ו' : 'ה'}`,
  ...
  ja: {
    ...
    noInterpellations: '現在、質問主意書はありません。',
    interpellationsBreakdown: (n: number): string => `野党議員が大臣の説明責任を求める${n}件の質問主意書を提出しました。`,
    ministerAccountability: '大臣の説明責任',
    ministerAccountabilityContext: (n: number): string => `今期${n}人の大臣が質問主意書に直面しており、野党の監視優先事項が浮き彫りになっています。`,
    targetMinister: '対象大臣',
    interpellationBy: '提出者',
    questioner: '質問者',
    oppositionOversight: '野党監視分析',
    readFullInterpellation: '質問主意書を見る',
    partyInterpellationsFiled: (party: string, n: number): string => `${party}：${n}件の質問主意書を提出`,
    twitterLabel1: '読了時間',
    twitterLabel2: '記事タイプ',
  ...
  ko: {
    ...
    noInterpellations: '현재 대정부 질의가 없습니다.',
    interpellationsBreakdown: (n: number): string => `야당 의원들이 장관 책임을 촉구하는 ${n}건의 대정부 질의를 제출했습니다.`,
    ministerAccountability: '장관 책임',
    ministerAccountabilityContext: (n: number): string => `이번 기간 ${n}명의 장관이 의회 질의를 받고 있으며, 이는 야당의 감시 우선순위를 보여줍니다.`,
    targetMinister: '대상 장관',
    interpellationBy: '제출자',
    questioner: '질의자',
    oppositionOversight: '야당 감시 분석',
    readFullInterpellation: '질의서 보기',
    partyInterpellationsFiled: (party: string, n: number): string => `${party}: ${n}건의 질의서 제출`,
    twitterLabel1: '읽기 시간',
    twitterLabel2: '기사 유형',
  ...
  zh: {
    ...
    noInterpellations: '目前没有可用的质询。',
    interpellationsBreakdown: (n: number): string => `反对党议员提出了${n}份质询，要求追究部长责任。`,
    ministerAccountability: '部长问责',
    ministerAccountabilityContext: (n: number): string => `本期有${n}名部长面临议会质询，突显了反对党的监督重点。`,
    targetMinister: '目标部长',
    interpellationBy: '提交人',
    questioner: '质询人',
    oppositionOversight: '反对党监督分析',
    readFullInterpellation: '查看质询',
    partyInterpellationsFiled: (party: string, n: number): string => `${party}：提交了${n}项质询`,
    twitterLabel1: '阅读时间',
    twitterLabel2: '文章类型',
    jobTitle: '政治情报分析师',
