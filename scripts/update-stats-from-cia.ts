...
import type { StatsData } from './load-cia-stats.js';
...
export interface LangDescriptions {
  ...
  aboutText?: string;
  mpTrackingText?: string;
  dashboardDescription?: string;
  riskHeatMapTitle?: string;
  featureListMPs?: string;
...
function updateHTMLFile(filePath: string, stats: StatsData): UpdateResult {
  ...
  try {
    ...
    console.error(`Failed to update ${filePath}: ${(err as Error).message}`);
    return { file: path.basename(filePath), changes: 0, updated: false, error: (err as Error).message };
  }
}
