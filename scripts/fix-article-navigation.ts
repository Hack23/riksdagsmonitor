...
export function transformContent(
  ...
  if (!hasTopnav) {
    ...
    // Note: the article/div tag may have attributes before class= (e.g. id="main-content")
    // and may carry additional classes (e.g. "news-article article-type-propositions")
    if (result.includes('</nav>')) {
      const navPattern = /((<\/nav>)([\s]*)(<(?:article|div)\b[^>]*?\bclass="(?:news-article(?:\s[^"]*)?|container)"))/s;
    ...
    // Pattern B: insert directly before <article/div with class="news-article ..." or "container">
    // Allows arbitrary attributes (e.g. id="main-content") before class=
    if (!inserted) {
      const articlePattern = /(<(?:article|div)\b[^>]*?\bclass="(?:news-article(?:\s[^"]*)?|container)")/;
