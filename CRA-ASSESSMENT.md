...
| **AI Content Manipulation** *(Art. 11)* | AI-authored analysis artifacts → rendered news HTML | M | M/H/L | `rehype-sanitize` allow-list at trust boundary (blocks `<script>`, `<iframe>`, inline handlers, `javascript:` URIs); Mermaid rendered in `securityLevel: 'strict'` (no eval); SHA-256 manifest per article for tamper detection; editorial quality checks via analysis-gate prompt; translation completeness validation via `scripts/validate-news-translations.ts` | M | [SECURITY_ARCHITECTURE.md](SECURITY_ARCHITECTURE.md) §2.5.1 + [THREAT_MODEL.md](THREAT_MODEL.md) + [WORKFLOWS.md](WORKFLOWS.md) |
