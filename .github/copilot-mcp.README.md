...
2. **Two endpoints to unify** — IMF exposes both Datamapper REST (`www.imf.org/external/datamapper/api/v1`) and SDMX 3.0 (`sdmxcentral.imf.org`); the CLI normalises both behind a single `getImfContext({domain, country, vintage})` interface
...
5. **Network egress is already audited** — agentic workflows declare `www.imf.org` and `sdmxcentral.imf.org` in their `network:` allowlist; an MCP server would relocate this surface without removing it
