...
- **Economic data security boundary** — IMF is the **primary economic-data source** ([`analysis/imf/`](../../analysis/imf/)); enforce egress allow-list `www.imf.org`, `api.imf.org`; SHA-256 payload pin + vintage-tagged supersedes-chain in cache; reject payloads >6 mo old without staleness annotation. STRIDE coverage in `THREAT_MODEL.md` §IMF (T-IMF-01..07). World Bank retained for governance/environment residue only. See [`.github/aw/ECONOMIC_DATA_CONTRACT.md`](../aw/ECONOMIC_DATA_CONTRACT.md) v3.2.
