...
- **IMF egress + monitoring** — pin `www.imf.org` and `api.imf.org` in `copilot-setup-steps.yml` network allow-list; emit `imf_api_requests_total{dataflow,status}` counter; ≤30 req/min self-imposed; exponential back-off on 429. IMF CLI smoke check in `mcp-setup.sh` (`tsx scripts/imf-fetch.ts --healthcheck`). IMF is the **primary economic-data source**; WB residue only. Hub: [`analysis/imf/`](../../analysis/imf/).
