...
- **IMF egress allow-list** — include `www.imf.org` and `api.imf.org` in CDN/WAF egress configs and in every workflow `network:` allowlist; preserve vintage-tagged cache across deploys. IMF is the **primary economic-data source** per [`.github/aw/ECONOMIC_DATA_CONTRACT.md`](../aw/ECONOMIC_DATA_CONTRACT.md) v2.1; WB residue only. Hub: [`analysis/imf/`](../../analysis/imf/).
