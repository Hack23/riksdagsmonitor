...
- **IMF test surface** — `tests/imf-client.test.ts`, `tests/imf-codes.test.ts`, `tests/imf-context.test.ts`, `tests/imf-inventory.test.ts` (13 schema assertions), `tests/economic-context-multi-provider.test.ts` (asserts IMF queried before WB). Future: `tests/imf-precedence-contract.test.ts`. IMF is the **primary economic-data source**; WB residue only. Hub: [`analysis/imf/`](../../analysis/imf/).
