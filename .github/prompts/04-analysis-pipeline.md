...
4. **Snapshot Pass-1** — copy every Pass-1 file into `$ANALYSIS_DIR/pass1/` before starting Pass 2: `mkdir -p "$ANALYSIS_DIR/pass1" && cp "$ANALYSIS_DIR"/*.md "$ANALYSIS_DIR/pass1/"`. The `pass1/` directory is the fallback evidence the gate uses when mtime windows are too tight. Do **not** stage `pass1/` in the PR (see `07-commit-and-pr.md`). *(Improvement-mode: the directory and its `.md` files are guaranteed present from the prior run, so the same `mkdir -p && cp` line works unchanged. The snapshot now captures the **pre-improvement** baseline, overwriting any earlier snapshot — the gate only needs the most recent baseline to compare Pass-2 deltas against.)*
