...
| 21 | `media-framing-analysis.md` | [`media-framing-analysis.md`](../../analysis/templates/media-framing-analysis.md) | **v2.0 (psyops + global + longevity).** ≥ 3 frame packages with Entman 4-function decomposition; cognitive-vulnerability map (Cialdini / Kahneman / Roozenbeek-van der Linden); DISARM TTP map (verbatim `T####`) or explicit no-signal finding; narrative-laundering chain (fringe → alt → politician → mainstream → international); source-ecology table from public registries (Nordicom, EUvsDisinfo, SÄPO); CIB ABCDE block (Camille François 2020); algorithmic-asymmetry rows with academic / transparency-report citations; ≥ 2 comparative-international frame cognates; strategic-doctrine detection table (firehose / doppelganger / gish gallop / reflexive control / active-measures spillover / interest-group capture / MAGA cognate); frame lifecycle + half-life + zombie-probability table; RRPA impact (Reach × Resonance × Persistence × Action) with dated action indicator; L1–L5 counter-resilience ladder per frame; longitudinal frame-record entry. Frame E (foreign overlay) only when state-affiliated or coordinated foreign amplification is observed. |
