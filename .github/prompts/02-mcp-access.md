...
- **Firewall egress**: `www.imf.org`, `api.imf.org`, `data.imf.org`, `api.imf.org` (already in every workflow's `network.allowed`). The TypeScript IMF client sends an explicit Riksdagsmonitor `User-Agent`; do not replace it with raw `node` / undici fetch calls because IMF Datamapper can reject those with HTTP 403.
