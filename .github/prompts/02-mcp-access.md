...
| `edit` | local helper | workflow `tools.edit:` | standard | **primary file-create / overwrite mechanism** — filesystem edits inside `$GITHUB_WORKSPACE`. Always use this for `.md` / `.json` / `.html` writes (translations, analyses, briefs, sidecars). See [`01-bash-and-shell-safety.md` §File creation & overwrite strategy](01-bash-and-shell-safety.md) for the strict Tier-1 (`edit`) → Tier-2 (`cat <<'QUOTED_EOF'`) hierarchy and the banned-pattern list (no `python3`, no `sed -i`, no unquoted heredocs for content). |
