...
# Stage individual article HTML files (the script generates them with type in filename)
git diff --name-only -- "news/" 2>/dev/null | while read -r f; do git add "$f" 2>/dev/null || true; done
git ls-files --others --exclude-standard -- "news/*.html" 2>/dev/null | while read -r f; do git add "$f" 2>/dev/null || true; done
