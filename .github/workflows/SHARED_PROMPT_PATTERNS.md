...
- [ ] "📊 Analysis & Sources" section present with GitHub links to all analysis files (auto-generated by `generateArticleHTML` via `analysisReferencesHtml`)
...
## 📊 ANALYSIS FILE GITHUB REFERENCES (AUTO-GENERATED)

> **AUTOMATED**: The "📊 Analysis & Sources" section is now **automatically generated** by `generateArticleHTML()` via the `analysisReferencesHtml` field. The `generateAnalysisReferencesHtml()` function in `scripts/analysis-references.ts` scans `analysis/daily/{date}/{subfolder}/` for all existing `.md` files and builds localized HTML with GitHub links. **AI agents do NOT need to manually insert this section** — it is included in every article by default.
...
````markdown
### Analysis File Reference Linking (Automated)

> ✅ **AUTO-GENERATED**: The `generateAnalysisReferencesHtml()` function automatically:
> 1. Scans `analysis/daily/{date}/{subfolder}/` for all `.md` files
> 2. Links to each found file on GitHub with localized labels (14 languages)
> 3. Includes a link to `analysis/methodologies/ai-driven-analysis-guide.md`
> 4. Adds a link to the `documents/` subdirectory when it exists
> 5. Only links to files that actually exist (no broken links)
>
> **AI agents should NOT manually add analysis references** — they are embedded by the template.

#### Reference Section Template (for reference — generated automatically)
...
<section class="analysis-references" aria-label="Analysis sources and methodology">
  ...
  <ul>
    ...
    <!-- ... all other .md files found in the analysis directory ... -->
...
#### Implementation (automated in TypeScript)

The analysis references are generated by `scripts/analysis-references.ts`:
