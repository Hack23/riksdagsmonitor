...
| **World Bank WDI / WGI (non-economic context)** | **A** | **1** | **PRIMARY** for governance (`*.EST`), environment (`EN.*`, `EG.*`, `AG.LND.FRST.ZS`), social/education residue (`SP.*`, `SH.*`, `SE.*`), defence historicals (`MS.MIL.*`), crime (`VC.IHR.PSRC.P5`) |

**Canonical OSINT rule:** IMF is graded A1/A2 for economic intelligence (macro, fiscal, monetary, external, trade, commodity, FX, interest rates) and is the source for every economic claim. World Bank is graded A1 for the governance, environment, social/education, defence-historical, and crime/justice context catalogued in [`analysis/worldbank/indicators-inventory.json`](../../../analysis/worldbank/indicators-inventory.json). See [`.github/aw/ECONOMIC_DATA_CONTRACT.md`](../../../.github/aw/ECONOMIC_DATA_CONTRACT.md) v3.0 and [`analysis/imf/`](../../../analysis/imf/).
