...
| **TB-6a: Agentic workflows → IMF (`c-cf/imf-data-mcp`, Python/`uvx`)** | Macro/fiscal/monetary queries to `data.imf.org` / `api.imf.org` via a third-party Python MCP server pulled from `git+https://github.com/c-cf/imf-data-mcp` through `pypi.org` + `files.pythonhosted.org` | **S**: Upstream repo compromise (commit replacement) or PyPI dep takeover; **T**: Tampering of IMF JSON responses in transit or at rest in `analysis/data/imf/`; **I**: Stale / mis-vintaged WEO projections cited as current; **D**: IMF rate-limit (~10 req / 5 s) causing workflow failure | Commit-SHA pin on `c-cf/imf-data-mcp` (Open_Source_Policy.md); firewall allowlist scoped to `data.imf.org` / `api.imf.org` / `www.imf.org` / `pypi.org` / `files.pythonhosted.org` only; SBOM entry + Dependabot-equivalent weekly review; cached responses under `analysis/data/imf/$year/` with mandatory `projectionVintage` stamp; batch-multi-country calls + 1 s sleep + 3× 429 back-off in `imf-client.ts` and Step 2.6 prompt |
