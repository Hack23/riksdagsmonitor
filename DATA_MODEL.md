...
> - 🧠 Added **§11 Analysis Artifact Data Model** — typed `ArtifactDefinition[]` for **23 artifacts** across Families A (9 core synthesis), B (2 structural metadata), C (5 strategic extensions), D (7 electoral/domain), E (per-document, variable). Required-Mermaid and required-Pass-2 booleans sourced from `scripts/agentic/artifact-inventory.ts`. Folder convention: `analysis/daily/$ARTICLE_DATE/$SUBFOLDER/`. Methodology-reflection JSON contract (ICD 203 audit) added.
> - 📰 Added **§12 News Corpus Data Model** — YAML frontmatter schema (defaults: `language='en'`, `layout='article'` from `scripts/render-lib/aggregator/frontmatter.ts`), 14-language file naming convention, `news/` directory structure. Current footprint: **3,953 `.html` files** under `news/` (verified 2026-05-06; up from the 2,669 reported at v1.1).
> - 🗂️ Added **§13 Political Intelligence Catalog Data Structure** — `scripts/political-intelligence/catalog.ts` outputs and `scripts/political-intelligence/daily-streams.ts` per-stream/per-day data shape, plus i18n leaves (artifact, methodology, page, stream, template).
> - 🌍 Formalised **IMF cache model** under §4: `analysis/data/imf/{indicator}/{country}.json` payload + `.meta.json` sidecar (provenance fields: provider, dataflow, indicator, vintage, retrieved_at). Documented SCB and World Bank cache patterns analogously, plus Riksbank, Statskontoret, RiR caches under `analysis/data/{provider}/`.
> - 🏷️ Added **`economicProvenance` block schema** (provider ∈ {`imf`, `scb`, `worldbank`, `riksbank`, `statskontoret`, `rir`}, dataflow, indicator, vintage, retrieved_at) and the rule that any economic data point in article frontmatter or body MUST carry one.
> - 📐 Updated **§5 Schemas &amp; Validation** with typed subpath exports (`./cia/*`, `./dashboards/*`, `./shared/*`, `./ui/*`) generated by `scripts/generate-types-from-cia-schemas.ts`, and root-level schemas (`schemas/article-types.schema.json`, `schemas/pir-status.schema.json`, `schemas/rir-followups-schema.json`).
> - 📚 Cross-referenced [`ARCHITECTURE.md` v2.5 §C4 Component view](ARCHITECTURE.md) for the analysis-gate (checks 1–9b, `PASS2_MTIME_THRESHOLD_MS = 180_000`) and the render-lib aggregator/cleaning/seo sub-trees.
>
