...
### Source Code ✅
✅ All scripts/*.js use 'no' (not 'nb')  
✅ All tests/*.js expect 'no' (with legacy 'nb' tolerance)  
✅ All locale mappings use 'no_NO' or 'no-NO'  
✅ All language arrays consistently use 'no'  
✅ All hreflang generation logic uses 'no'

### Generated HTML Files ⚠️ IN PROGRESS
✅ news/index.html and news/index_sv.html updated to hreflang="no"  
⚠️ Legacy article HTML files still contain hreflang="nb" (will be fixed when regenerated)  
⚠️ Sitemaps will use 'no' when regenerated

**Note**: Some committed HTML files (news articles, sitemaps) still contain `hreflang="nb"` because they were generated before this standardization. These will automatically use `hreflang="no"` when regenerated by the updated scripts.

## Status

✅ **Source Code**: All 'nb' references removed  
✅ **Tests**: Updated to expect 'no' with legacy 'nb' tolerance  
⚠️ **Generated HTML**: Partial (news indexes updated, articles pending regeneration)  
