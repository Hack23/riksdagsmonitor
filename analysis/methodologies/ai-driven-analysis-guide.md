...
| `media-framing-analysis.md` | **v2.0** — frame packages with Entman 4-functions, cognitive-vulnerability map, DISARM TTP map, narrative-laundering chain (fringe → alt → politician → mainstream → international), source-ecology table, CIB ABCDE block, algorithmic asymmetry, ≥ 2 comparative-international cognates, strategic-doctrine detection, frame lifecycle / longevity, RRPA impact, L1–L5 counter-resilience ladder, longitudinal frame-record entry | [`media-framing-analysis.md`](../templates/media-framing-analysis.md) | Outside-In Thinking + Red Cell + ACH (≥ 3 hypotheses for any state-attribution claim) + Indicators & Signposts |
