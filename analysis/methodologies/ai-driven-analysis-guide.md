...
- **IMF data fallback hierarchy** (codified in `scripts/imf-fetch.ts`):
  1. **Live fetch** — `tsx scripts/imf-fetch.ts weo --country SWE --indicator X --persist` → fresh data + persisted cache.
  2. **Cached fallback** — when live fetch fails (network block, timeout, API error), the script automatically loads from `analysis/data/imf/{indicator}/{country}.json`. The output includes `_fallback: true` and `_vintageAnnotation`.
  ...
  5. **World Bank prohibition** — per ECONOMIC_DATA_CONTRACT v2.1, World Bank is reserved for governance (WGI), environment, and social residue. Using WB for GDP/growth/debt/inflation triggers the `imf-primary-violation` warning in `validate-methodology-reflection.ts`.
