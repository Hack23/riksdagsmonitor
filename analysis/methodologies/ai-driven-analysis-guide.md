...
| `coalition-mathematics.md` | Current seat map + pivotal votes + Sainte-Laguë scenarios; stable structure regardless of daily contentiousness. **MUST** include a voting-record table sourced from `fetch-voting-records` output (`data/voteringar/{date}/{bet}.json`) for every betänkande cited, or one of the explicit annotations emitted by the script: `<!-- vote-pending: {bet} -->` when `status: "vote_pending"` (vote not yet taken), `<!-- vote-not-found: {bet} -->` when `status: "not_found"` (MCP returned successfully with zero data — e.g. referral or procedural vote), or `<!-- vote-fetch-error: {bet} -->` when `status: "error"` (transient MCP/network failure; rerun once the service is back). | [`coalition-mathematics.md`](../templates/coalition-mathematics.md) | Morphological |
