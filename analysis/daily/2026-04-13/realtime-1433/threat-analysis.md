# 🔴 Political Threat Analysis — 2026-04-13 Realtime Monitor

**Generated**: 2026-04-13 14:33 UTC | **Documents**: 9 | **Confidence**: HIGH

## Threat Taxonomy

```mermaid
graph TD
    subgraph "Political Threat Taxonomy"
        ROOT["Spring Budget 2026 Threats"]
        ROOT --> FISCAL["💰 Fiscal Policy Threats"]
        ROOT --> GEOPOLITICAL["🌍 Geopolitical Threats"]
        ROOT --> SOCIAL["👥 Social Policy Threats"]
        FISCAL --> F1["Opposition alternative budget<br/>Probability: HIGH"]
        FISCAL --> F2["Forecast credibility attack<br/>Probability: MEDIUM"]
        FISCAL --> F3["EU fiscal rule tension<br/>Probability: LOW"]
        GEOPOLITICAL --> G1["Russian escalation response<br/>Probability: LOW-MEDIUM"]
        GEOPOLITICAL --> G2["NATO burden-sharing debate<br/>Probability: MEDIUM"]
        SOCIAL --> S1["Reception law legal challenge<br/>Probability: MEDIUM"]
        SOCIAL --> S2["Cost-of-living discontent<br/>Probability: HIGH"]
    end
    style ROOT fill:#1565C0,stroke:#FFFFFF,color:#FFFFFF
    style FISCAL fill:#D32F2F,stroke:#424242,color:#FFFFFF
    style GEOPOLITICAL fill:#FF9800,stroke:#424242,color:#FFFFFF
    style SOCIAL fill:#FFC107,stroke:#424242,color:#000000
    style F1 fill:#D32F2F,stroke:#424242,color:#FFFFFF
    style F2 fill:#FF9800,stroke:#424242,color:#FFFFFF
    style F3 fill:#FFC107,stroke:#424242,color:#000000
    style G1 fill:#FFC107,stroke:#424242,color:#000000
    style G2 fill:#FF9800,stroke:#424242,color:#FFFFFF
    style S1 fill:#FF9800,stroke:#424242,color:#FFFFFF
    style S2 fill:#D32F2F,stroke:#424242,color:#FFFFFF
```

| Threat | Category | Actor | Probability | Impact | dok_id |
|--------|----------|-------|-------------|--------|--------|
| S alternative spring budget | Fiscal | Socialdemokraterna | HIGH | Strategic | HD03100 |
| Cost-of-living discontent despite relief | Social | Citizens | HIGH | Electoral | HD03236 |
| VP forecast credibility challenged | Fiscal | Opposition + Media | MEDIUM | Institutional | HD03100, HD03241 |
| NATO deployment escalation | Geopolitical | Russia | LOW-MEDIUM | Security | HD01UFöU3 |
| Reception law EU legal challenge | Social | EU/Civil Society | MEDIUM | Legal | HD024076 |
| Climate policy contradiction exposed | Fiscal | MP, EU | MEDIUM | Policy coherence | HD03236 |
