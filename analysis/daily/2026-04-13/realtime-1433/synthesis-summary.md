...
graph TD
    subgraph "📊 Daily Political Intelligence Dashboard"
        ...
        subgraph "🔒 Sensitivity"
            S1["🟡 SENSITIVE: 7 documents"]
            S2["🔴 RESTRICTED: 1 document"]
            S3["🟢 PUBLIC: 1 document"]
        end
        subgraph "⚡ Risk Level"
            R1["🔴 HIGH: Energy/fiscal policy"]
            R2["🟠 MEDIUM: Defence/NATO"]
            R3["🟡 LOW: Procedural"]
        end
        subgraph "📈 Significance"
            SIG1["🔴 CRITICAL: Vårpropositionen HD03100"]
            SIG2["🟠 HIGH: Extra budget HD03236"]
...
```mermaid
graph TD
    subgraph "SWOT Quadrant Mapping"
        direction TB
        subgraph "💪 Strengths"
            STR1["Coalition fiscal coordination<br/>6 synchronized propositions"]
            STR2["NATO integration milestone<br/>Swedish forces to Finland"]
            STR3["Energy crisis response<br/>Concrete fuel/electricity relief"]
        end
        subgraph "⚠️ Weaknesses"
            W1["Easter timing limits scrutiny<br/>VP released during recess"]
            W2["Fuel tax cuts vs climate<br/>Green Deal tension"]
            W3["Ad-hoc emergency budgets<br/>Fiscal framework erosion"]
        end
        subgraph "🎯 Opportunities"
            O1["VP debate frames autumn<br/>2026 budget priorities"]
            O2["Nordic defence showcase<br/>NATO credibility signal"]
            O3["Opposition policy exposure<br/>S must present alternatives"]
        end
        subgraph "🔥 Threats"
            T1["Opposition fiscal challenge<br/>S alternative spring budget"]
            T2["Economic forecast risk<br/>Global trade uncertainty"]
