...
```mermaid
mindmap
  root((KU38: Parliamentary Process))
    🏛️ Parliamentary Reform
      ::icon(⚖️)
      MP working conditions
    ...
    🔄 Recent KU Work
      ::icon(📊)
      KU31: Minority languages
      KU29: Public administration
      KU30: Constitutional questions
```
...
```mermaid
quadrantChart
    title Government SWOT — Parliamentary Process (KU38)
    x-axis "Weaknesses" --> "Strengths"
    y-axis "Threats" --> "Opportunities"
    quadrant-1 "Democratic credibility"
    quadrant-2 "Reform leadership"
    quadrant-3 "Procedural disputes"
    quadrant-4 "Opposition critique"
    "Modernisation signal": [0.70, 0.75]
    "Cross-party support": [0.65, 0.60]
    "Implementation risk": [0.35, 0.40]
    "Partisan framing": [0.30, 0.30]
```
...
```mermaid
flowchart LR
    A[KU38 Published<br/>2026-03-30] --> B[Chamber Debate<br/>Scheduled]
    B --> C{Voting}
