...
```mermaid
quadrantChart
    title GUTE II Defense Contract — SWOT Quadrant
    ...
    quadrant-3 "Avoid"
    quadrant-4 "Defend"
    "Industrial base": [0.85, 0.75]
    "Counter-drone gap": [0.9, 0.65]
    "Supply diversification": [0.7, 0.5]
    "NATO integration": [0.6, 0.85]
    "Export potential": [0.65, 0.9]
    "Budget pressure": [0.25, 0.45]
    "Delivery delays": [0.3, 0.35]
    "Electoral backlash": [0.2, 0.2]
```
...
```mermaid
flowchart TD
    A["GUTE II Contract Signed<br/>SEK 8.7B"] --> B["Production Ramp-up<br/>Saab + BAE + SISU + Nammo"]
    B --> C{"Delivery On Schedule?"}
    C -->|Yes| D["Enhanced Air Defense<br/>Counter-Drone Capability"]
    C -->|No| E["Capability Gap Persists<br/>Budget Escalation Risk"]
