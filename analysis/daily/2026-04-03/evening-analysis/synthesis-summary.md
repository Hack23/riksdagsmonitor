...
```mermaid
graph LR
    A[18 Documents] --> B{Sensitivity}
    B -->|HIGH 4| C[Defense & Immigration]
    B -->|MEDIUM 8| D[Education & Health]
    B -->|LOW 6| E[Admin & Routine]
    C --> F{Risk Level}
    D --> F
    E --> F
    F -->|Critical 2| G[🔴 Breaking]
    F -->|High 4| H[🟠 Priority]
    F -->|Medium 8| I[🟡 Standard]
    F -->|Low 4| J[🟢 Monitor]
    G --> K[Editorial Decision]
    H --> K
    I --> K
    J --> K
...
graph TB
    subgraph Strengths
        S1[Defense Investment SEK 8.7B]
        S2[Cybersecurity Modernization]
        S3[NATO Integration Deepening]
    end
    subgraph Weaknesses
        W1[Constitutional Tension on Deportation]
        W2[Education Policy Overload]
        W3[Opposition Fragmentation on Defense]
    end
    subgraph Opportunities
        O1[EU NIS2 Compliance Leadership]
        O2[Pre-election Security Narrative]
        O3[Cross-party Defense Consensus]
    end
    subgraph Threats
        T1[Arms Export Ethics Backlash]
