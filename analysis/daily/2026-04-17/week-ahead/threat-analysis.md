...
```mermaid
graph TD
    A[Political Threats Apr 17-24] --> B[Constitutional Accountability]
    A --> C[Coalition Stability]
    A --> D[Legislative Agenda]
    A --> E[Electoral Positioning]
    
    B --> B1[KU scrutiny exposing governance gaps]
    B --> B2[Svantesson hearing: economic inequality narrative]
    B --> B3[Wallström hearing: foreign policy contrast]
    
    C --> C1[L under pressure on gender equality]
    C --> C2[SD comfort with welfare reduction differs from L]
    C --> C3[M-L-KD-SD coherence tested publicly]
    
    D --> D1[Opposition reservations on CU housing reforms]
    D --> D2[Women shelter closures blocking social legislation narrative]
    D --> D3[Wage transparency timeline pressure]
    
    E --> E1[S building E2026 case via interpellations]
