...
```mermaid
graph LR
    A[HD01KU32 — Media Accessibility Grundlag] --> B{Sensitivity}
    B -->|"🔴 RESTRICTED"| C[Constitutional Amendment<br/>TF + YGL]

    A --> D{Domain}
    D --> E[Constitutional Law]
    D --> F[Media / Press Freedom]
    D --> G[Disability Rights]
    D --> H[EU Compliance]

    A --> I{Urgency}
    I -->|"🟠 URGENT"| J[EU EAA deadline pressure<br/>Infringement risk late-2027]

    A --> K{Political Conflict}
    K -->|"🟢 LOW"| L[5 technical reservations<br/>Cross-bloc support]
...
```mermaid
graph TD
    subgraph "📻 HD01KU32 — Media Accessibility SWOT"
        S["💪 STRENGTHS<br/>• EU EAA compliance enabler<br/>• Cross-bloc support (only 5 reservations)<br/>• ~500K disabled Swedes benefit<br/>• Technical, non-divisive amendment<br/>• Disability-rights NGOs aligned"]
        W["⚠️ WEAKNESSES<br/>• Vilande — election creates 5+ month uncertainty<br/>• Narrow constitutional exception complex to draft<br/>• Media industry compliance costs disputed<br/>• Low public visibility = low political pressure"]
        O["🎯 OPPORTUNITIES<br/>• Sweden accessibility leadership in EU<br/>• Foundation for AI-driven accessibility tech<br/>• Cross-Nordic harmonisation potential<br/>• Strengthens CRPD Article 9 implementation"]
        T["🚨 THREATS<br/>• EU infringement if second reading delayed<br/>• Media industry lobbies for narrow scope<br/>• Low salience = easily deprioritised post-election<br/>• Constitutional amendment rollback pressure"]
    end
