...
```mermaid
graph LR
    A[HD03235] --> B{Sensitivity}
    B -->|"🟡 SENSITIVE"| D[Migration & Criminal Justice]

    A --> G{Domain}
    G --> H["Migration & Justice (MIG/JUS)"]

    A --> I{Urgency}
    I -->|"🟠 URGENT"| K[Active legislative priority — referred to SfU]
...
```mermaid
quadrantChart
    title Political Impact — Deportation Rules
    ...
    "Delivers on Tidö Agreement": [0.2, 0.8]
    "ECHR compliance risk": [0.3, 0.2]
    "S must navigate tough-on-crime stance": [0.65, 0.3]
    "V/MP mobilize human rights base": [0.8, 0.7]
```
