...
```mermaid
graph LR
    A[HD01JuU15] --> B{Sensitivity}
    B -->|"🟢 PUBLIC"| E[Standard Parliamentary]

    A --> G{Domain}
    G --> H["Justice & Corrections (JUS)"]

    A --> I{Urgency}
    I -->|"🔵 ELEVATED"| L[Committee report — monitoring this week]
...
```mermaid
quadrantChart
    title Political Impact — Criminal Justice Report
    ...
    "Addresses prison overcrowding narrative": [0.25, 0.6]
    "Capacity gaps persist": [0.35, 0.3]
    "S highlights rehabilitation failures": [0.7, 0.55]
    "V demands alternatives to prison": [0.85, 0.65]
```
