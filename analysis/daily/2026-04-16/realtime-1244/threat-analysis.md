...
```mermaid
graph TD
    ROOT["🎯 Prop. 246 Reform Fails<br/>(Goal: prevent or undermine)"] --> A["Constitutional<br/>Challenge"]
    ROOT --> B["Implementation<br/>Failure"]
    ROOT --> C["Electoral<br/>Reversal"]
    ROOT --> D["International<br/>Pressure"]
    
    A --> A1["UN CRC objection<br/>GC24: age lowering<br/>'never acceptable'"]
    A --> A2["Lagrådet review<br/>Proportionality test<br/>vs incorporated CRC"]
    A --> A3["ECHR application<br/>Art. 3 (inhuman treatment)<br/>if 13-yr-old incarcerated"]
    
    B --> B1["SiS capacity crisis<br/>>90% occupancy<br/>No expansion plan"]
    B --> B2["Municipal services<br/>overloaded<br/>No budget allocated"]
    B --> B3["31 law amendments<br/>Implementation errors<br/>in 3.5-month timeline"]
    
    C --> C1["S wins 2026 election<br/>Reverses before<br/>sunset (2031)"]
    C --> C2["S tables alternative<br/>Evidence-based approach<br/>gains voter support"]
    
    D --> D1["Council of Europe<br/>Commissioner criticism"]
...
```mermaid
graph TB
    subgraph "🔴 Critical (Score 20+)"
        T_IMPL["Implementation Failure<br/>SiS+Municipal: 20"]
        T_CONST["Constitutional Challenge<br/>UN CRC: 20"]
    end
    
    subgraph "🟠 High (Score 12-19)"
        T_ELEC["Electoral Backlash<br/>S 88/88 Nej: 16"]
        T_INTL["International Isolation<br/>Nordic outlier: 15"]
    end
    
    subgraph "🟡 Medium (Score 6-11)"
        T_LAGR["Lagrådet Review<br/>Proportionality: 9"]
        T_RECID["Recidivism Evidence<br/>Research: 9"]
    end
    
    subgraph "🟢 Low (Score 1-5)"
        T_SD["SD Dissent<br/>Stable: 3"]
    end
    
    T_IMPL -.->|"feeds"| T_ELEC
    T_CONST -.->|"amplifies"| T_INTL
