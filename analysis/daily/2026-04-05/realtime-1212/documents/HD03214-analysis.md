...
```mermaid
graph TD
    subgraph "🔒 HD03214 — Cybersecurity Center Strengthening"
        PROP["📋 Prop HD03214<br/>Cybersecurity Center"]
        MSB["🛡️ MSB<br/>Civil Defense"]
        FRA["🔍 FRA<br/>Signals Intelligence"]
        MUST["⚔️ MUST<br/>Military Intelligence"]
        NCSC["🏛️ NCSC-SE<br/>National Cybersecurity Center"]
        PRIV["🏭 Private Sector<br/>Critical Infrastructure"]
        NATO["🌐 NATO CCDCOE<br/>Cyber Defence"]
    end

    PROP -->|"expands mandate"| NCSC
    NCSC -->|"coordinates"| MSB
    NCSC -->|"coordinates"| FRA
    NCSC -->|"coordinates"| MUST
    NCSC -->|"requires info sharing"| PRIV
    NCSC -->|"integrates with"| NATO

    style PROP fill:#0d6efd,color:#fff
    style NCSC fill:#28a745,color:#fff
    style MSB fill:#fd7e14,color:#fff
    style FRA fill:#dc3545,color:#fff
    style MUST fill:#dc3545,color:#fff
    style PRIV fill:#ffc107,color:#000
