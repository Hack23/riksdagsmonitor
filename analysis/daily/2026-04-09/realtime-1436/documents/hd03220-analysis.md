...
```mermaid
graph LR
    A["📄 HD03220"] --> B{Sensitivity}
    B -->|"🟡 SENSITIVE"| D["Defence/NATO deployment"]

    A --> G{Domain}
    G --> H["Defence & Security (DEF)"]
...
```mermaid
quadrantChart
    title NATO Finland Deployment Impact
    ...
    "NATO credibility boost": [0.2, 0.8]
    "Nordic defense leadership": [0.3, 0.9]
    "SD base tension": [0.3, 0.2]
    "S foreign policy critique limited": [0.8, 0.3]
```
...
graph LR
    ...
    NI --> NI1["Low — broad consensus limits disinformation risk"]
...
graph TD
    subgraph "📄 Document Impact Assessment"
        DOC["📄 HD03220<br/>NATO Finland Deployment"]
    end

    subgraph "🏛️ Political Actors"
        GOV["🏛️ Government<br/>M + KD + L (+ SD)"]
        OPP["⚖️ Opposition<br/>S supports, V/MP opposed"]
    end

    subgraph "👥 Society & Economy"
        CIT["👥 Citizens<br/>National security enhanced"]
        ECO["💰 Economic<br/>Defence industry boost"]
    end
