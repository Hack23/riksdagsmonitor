...
```mermaid
graph TD
    subgraph Political Risk Landscape
        R1[RSK-001 Taiwan WHA diplomatic exposure L2 x I4 = 8]
        R2[RSK-002 Climate policy delay L2 x I2 = 4]
        R3[RSK-003 AF reform criticism L2 x I2 = 4]
        R4[RSK-004 Gotland freight issue L1 x I1 = 1]
        R5[RSK-005 PAO-stod accountability L1 x I2 = 2]
    end

    subgraph Risk Score Tiers
        TM[MEDIUM Score 5 to 9]
        TL[LOW Score 1 to 4]
    end
...
timeline
    ...
    section Near-term 0 to 6 months
        Spring budget debate : April 13 2026
        WHA session : May 2026
    section Medium-term 6 to 18 months
        Pre-election positioning : S building campaign arguments
    section Long-term 18+ months
        Next General Election : September 2026
```
...
```mermaid
flowchart TD
    TRIGGER[TRIGGER: Sweden takes strong pro-Taiwan WHA position L2 x I4 = 8]
    TRIGGER --> FIRST[1ST ORDER: PRC diplomatic protest note to Swedish embassy L3 x I3 = 9]
...
graph LR
    subgraph Risk Interconnection Map
        COA[Coalition Stability]
        POL[Policy Implementation]
        BUD[Budget Fiscal]
        ELE[Electoral Risk]
        EXT[External International]
    end

    COA -->|SD support enables legislation| POL
    COA -->|Majority secures budget| BUD
    POL -->|Delivery gaps feed opposition| ELE
