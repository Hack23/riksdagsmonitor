...
graph TD
    subgraph Daily Political Intelligence Dashboard
        ...
        subgraph Risk
            RSK[Overall Risk LOW - Routine written questions]
        end
        subgraph Threat
            THR[Threat Level LOW - Standard parliamentary activity]
        end
        subgraph Significance
            SIG[Top Significance 3/10 - Monitor only]
        end
    end
    ...
    style SIG fill:#28a745,color:#fff
...
```mermaid
graph LR
    subgraph Government Coalition Assessment
        GS[Strengths 3 entries - Dominant: Coalition foreign policy alignment]
        GW[Weaknesses 4 entries - Critical: Climate policy delay]
        GO[Opportunities 3 entries - Top: Spring budget positioning]
        GT[Threats 2 entries - Top: S election ammunition]
    end
